import type { Entity } from "../../../schema/schema.general";
...
function main(context: Entity.Script.Babylon.I_Context) {
	...
	const hooks = {
		...
		onScriptInitialize: (
			...
			console.log("Script initialized for entity:", entity.general__entity_id);
		...
		onEntityUpdate: (updatedEntity: Entity.I_Entity) => {
			...
			const transformPosition = updatedEntity.meta__data
				.transform__position as PositionData;
			const entityModel = updatedEntity.meta__data.entity_model as EntityModel;
			const position = transformPosition || entityModel?.transform__position;
