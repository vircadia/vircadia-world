...
describe("Database Tests", () => {
    ...
    describe("Test Environment Setup", () => {
        ...
        test("should handle unset agent context gracefully", async () => {
            ...
            await sql`SELECT set_config('app.current_agent_id', NULL, true)`;
        ...
        test("should handle valid agent context", async () => {
            // First set the agent context properly using the auth function
            await sql`SELECT auth.set_agent_context(${admin.sessionId}, ${admin.token})`;
    ...
    //         await sql`SET SESSION CHARACTERISTICS AS TRANSACTION ISOLATION LEVEL REPEATABLE READ`;
    //     });
    ...
    //         const [tickResult] = await sql<[Tick.I_TickState]>`
    //             SELECT * FROM tick.capture_tick_state('NORMAL')
    //         `;

    //         // Verify tick metadata structure
    //         expect(tickResult.tickData).toBeDefined();
    //         expect(tickResult.tickData.tickNumber).toBeGreaterThan(0);
    //         expect(tickResult.tickData.tickStartTime).toBeDefined();
    //         expect(tickResult.tickData.tickEndTime).toBeDefined();
    //         expect(tickResult.tickData.tickDurationMs).toBeGreaterThanOrEqual(
    //             0,
    //         );
    //         expect(tickResult.tickData.isDelayed).toBeDefined();
    //         expect(tickResult.tickData.headroomMs).toBeGreaterThanOrEqual(0);

    //         // Verify entity updates
    //         expect(Array.isArray(tickResult.entityUpdates)).toBe(true);
    //         const entityUpdate = tickResult.entityUpdates.find(
