...
let azureAuthService: AzureADAuthService;

// Mock data for Azure AD responses - use a function to generate unique data per test
const getMockUserInfo = (): I_UserInfo => {
    const uniqueId = randomUUID();
    return {
        id: `mock-azure-user-${uniqueId}`,
        email: `${DB_TEST_PREFIX}azure.user.${uniqueId}@test.com`,
        displayName: `${DB_TEST_PREFIX}Azure Test User ${uniqueId}`,
        givenName: "Test",
        surname: "User",
        userPrincipalName: `${DB_TEST_PREFIX}azure.user.${uniqueId}@test.com`,
    };
};

// Create a base mock user for tests that don't need unique data
const mockUserInfo: I_UserInfo = getMockUserInfo();

const mockTokenResponse: I_TokenResponse = {
...
// Mock MSAL methods
const mockGetAuthCodeUrl = mock(() =>
    Promise.resolve("https://login.microsoftonline.com/mock-auth-url"),
);
const mockAcquireTokenByCode = mock(() =>
    Promise.resolve({
        ...
        account: {
            ...
            username: mockUserInfo.email,
            localAccountId: mockUserInfo.id,
        },
    }),
);
        ...
        return Promise.resolve({
            ...
            json: () =>
                Promise.resolve({
                    ...
                    surname: mockUserInfo.surname,
                    userPrincipalName: mockUserInfo.userPrincipalName,
                }),
        } as Response);
    }
...
describe("Azure AD Authentication", () => {
    beforeAll(async () => {
        ...
        `;
        // Also clean up any test Azure users by their specific email patterns
        await superUserSql`
            DELETE FROM auth.agent_profiles 
            WHERE auth__email LIKE ${`${DB_TEST_PREFIX}%`}
               OR profile__username LIKE ${`${DB_TEST_PREFIX}%`}
        `;
