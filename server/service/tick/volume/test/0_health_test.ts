...
describe("World Script Web Manager - HEALTH", () => {
    test("Health Check - Stats Endpoint", async () => {
        ...
        const statsEndpoint = `http://${VircadiaConfig.SERVER.VRCA_SERVER_SERVICE_TICK_HOST_CLUSTER}:${VircadiaConfig.SERVER.VRCA_SERVER_SERVICE_TICK_PORT_CLUSTER}${Communication.REST.Endpoint.STATS.path}`;
