...
const DEFAULT_POSTGRES_SEEDS_DIR = VircadiaConfig_Server.POSTGRES.SEED_PATH
	? path.resolve(VircadiaConfig_Server.POSTGRES.SEED_PATH)
	: path.join(
			dirname(fileURLToPath(import.meta.url)),
...
async function runDockerCommand(args: string[]) {
	const processEnv = {
		...process.env,
		POSTGRES_DB: VircadiaConfig_Server.POSTGRES.DATABASE,
		POSTGRES_USER: VircadiaConfig_Server.POSTGRES.USER,
		POSTGRES_PASSWORD: VircadiaConfig_Server.POSTGRES.PASSWORD,
		POSTGRES_PORT: VircadiaConfig_Server.POSTGRES.PORT.toString(),
		POSTGRES_HOST: VircadiaConfig_Server.POSTGRES.HOST,
		CONTAINER_NAME: VircadiaConfig_Server.CONTAINER_NAME,
	...
	const checkPgweb = async (): Promise<{
		...
		try {
			const response = await fetch(
				`http://localhost:${VircadiaConfig_Server.PGWEB.PORT}`,
			);
...
export async function up() {
	log({
		message: `Starting ${VircadiaConfig_Server.CONTAINER_NAME} services...`,
		type: "info",
		suppress: VircadiaConfig_Server.SUPPRESS,
		debug: VircadiaConfig_Server.DEBUG,
	});
	...
	log({
		message: `Waiting for ${VircadiaConfig_Server.CONTAINER_NAME} services to be ready...`,
		type: "info",
		suppress: VircadiaConfig_Server.SUPPRESS,
		debug: VircadiaConfig_Server.DEBUG,
	});
	if (!(await waitForHealthy())) {
		throw new Error(
			`${VircadiaConfig_Server.CONTAINER_NAME} services failed to start properly after 30 seconds`,
		);
	}
	log({
		message: `${VircadiaConfig_Server.CONTAINER_NAME} services are ready`,
		type: "success",
		suppress: VircadiaConfig_Server.SUPPRESS,
		debug: VircadiaConfig_Server.DEBUG,
	});
	...
	try {
		...
		log({
			message: `Running initial migrations for ${VircadiaConfig_Server.CONTAINER_NAME}...`,
			type: "info",
			suppress: VircadiaConfig_Server.SUPPRESS,
			debug: VircadiaConfig_Server.DEBUG,
		});
	...
	log({
		message: `${VircadiaConfig_Server.CONTAINER_NAME} container started successfully.`,
		type: "success",
		suppress: VircadiaConfig_Server.SUPPRESS,
		debug: VircadiaConfig_Server.DEBUG,
	});
...
export async function down() {
	log({
		message: `Stopping ${VircadiaConfig_Server.CONTAINER_NAME} container...`,
		type: "info",
		suppress: VircadiaConfig_Server.SUPPRESS,
		debug: VircadiaConfig_Server.DEBUG,
	});
	...
	log({
		message: `${VircadiaConfig_Server.CONTAINER_NAME} container stopped`,
		type: "success",
		suppress: VircadiaConfig_Server.SUPPRESS,
		debug: VircadiaConfig_Server.DEBUG,
	});
...
export async function rebuildContainer() {
	log({
		message: `Performing hard reset of ${VircadiaConfig_Server.CONTAINER_NAME} container...`,
		type: "info",
		suppress: VircadiaConfig_Server.SUPPRESS,
		debug: VircadiaConfig_Server.DEBUG,
	});
	...
	log({
		message: `Container ${VircadiaConfig_Server.CONTAINER_NAME} removed, rebuilding...`,
		type: "info",
	});
	await runDockerCommand(["up", "-d", "--build"]);
	log({
		message: `${VircadiaConfig_Server.CONTAINER_NAME} container reset complete, running migrations...`,
		type: "info",
		suppress: VircadiaConfig_Server.SUPPRESS,
		debug: VircadiaConfig_Server.DEBUG,
	});
	...
	log({
		message: `${VircadiaConfig_Server.CONTAINER_NAME} database migrations applied`,
		type: "success",
		suppress: VircadiaConfig_Server.SUPPRESS,
		debug: VircadiaConfig_Server.DEBUG,
	});
...
export async function softResetDatabase() {
	log({
		message: `Performing soft reset of ${VircadiaConfig_Server.CONTAINER_NAME} database...`,
		type: "info",
		suppress: VircadiaConfig_Server.SUPPRESS,
		debug: VircadiaConfig_Server.DEBUG,
	});
	...
	log({
		message: `${VircadiaConfig_Server.CONTAINER_NAME} database reset complete. Running migrations...`,
		type: "info",
		suppress: VircadiaConfig_Server.SUPPRESS,
		debug: VircadiaConfig_Server.DEBUG,
	});
	...
	log({
		message: `${VircadiaConfig_Server.CONTAINER_NAME} container soft reset complete.`,
		type: "success",
		suppress: VircadiaConfig_Server.SUPPRESS,
		debug: VircadiaConfig_Server.DEBUG,
	});
...
export async function migrate(data: {
	...
	log({
		message: `Running ${VircadiaConfig_Server.CONTAINER_NAME} database migrations...`,
		type: "info",
		suppress: VircadiaConfig_Server.SUPPRESS,
		debug: VircadiaConfig_Server.DEBUG,
	});
	...
	log({
		message: `${VircadiaConfig_Server.CONTAINER_NAME} container migrations applied`,
		type: "success",
	});
}
...
export async function seed(data: {
	...
	log({
		message: `${VircadiaConfig_Server.CONTAINER_NAME} container seeds applied.`,
		type: "success",
	});
}
...
export async function restart() {
	log({
		message: `Restarting ${VircadiaConfig_Server.CONTAINER_NAME} container...`,
		type: "info",
		suppress: VircadiaConfig_Server.SUPPRESS,
		debug: VircadiaConfig_Server.DEBUG,
	});
...
export async function generateDbConnectionString(): Promise<string> {
	...
	return `postgres://${VircadiaConfig_Server.POSTGRES.USER}:${VircadiaConfig_Server.POSTGRES.PASSWORD}@${VircadiaConfig_Server.POSTGRES.HOST}:${VircadiaConfig_Server.POSTGRES.PORT}/${VircadiaConfig_Server.POSTGRES.DATABASE}`;
}
		...
		switch (command) {
			...
			case "container:health": {
				...
				log({
					...
					type: health.services.postgres.isHealthy ? "success" : "error",
				});
				log({
			...
			case "container:pgweb:access-command": {
				...
				const isRemoteHost = !["localhost", "127.0.0.1"].includes(
					VircadiaConfig_Server.POSTGRES.HOST,
				);

				const accessMessage = isRemoteHost
					? `1. First create SSH tunnel:\nssh -L ${VircadiaConfig_Server.PGWEB.PORT}:localhost:${VircadiaConfig_Server.PGWEB.PORT} username@${VircadiaConfig_Server.POSTGRES.HOST}\n\n2. Then access pgweb at:`
					: "Access pgweb at:";
