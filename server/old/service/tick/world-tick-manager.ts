...
export class WorldTickManager {
	...
	async initialize() {
		try {
			...
			this.proxyUserSql = await PostgresClient.getInstance().getProxyClient();
	...
	private async processTick(syncGroup: string) {
		...
		log({
			...
			data: {
				"Entity Changes": result?.tick_data.tick__entity_states_processed,
				"Script Changes": result?.tick_data.tick__script_states_processed,
