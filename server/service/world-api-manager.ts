...
class WorldRestManager {
	...
	async handleRequest(req: Request): Promise<Response> {
		...
		switch (true) {
			case path === Communication.REST.Endpoint.AUTH_SESSION_VALIDATE.path &&
				req.method === "GET":
				return await this.handleSessionValidate(req);

			case path === Communication.REST.Endpoint.AUTH_SESSION_LOGOUT.path &&
				req.method === "POST":
		...
		if (result) {
			...
			if (
				...
				const updatePromises = Array.from(sessionUpdates.entries()).map(
					([sessionId, updates]) => {
						const session = this.wsManager.activeSessions.get(sessionId);
						if (session?.ws) {
...
class WorldWebSocketManager {
	...
	private async handleQuery(
		...
		try {
			...
			const results = await this.sql.begin(async (sql) => {
				...
				return await sql.unsafe(message.query, message.parameters || []);
			});
			...
			const errorMsg =
				Communication.WebSocket.createMessage<Communication.WebSocket.ErrorResponseMessage>(
					{
						type: Communication.WebSocket.MessageType.ERROR_RESPONSE,
	...
	handleConnection(
		...
		this.tokenMap.set(ws, (ws as ServerWebSocket<WebSocketData>).data.token);
	...
	async handleMessage(
		...
		try {
			...
			switch (data.type) {
				case Communication.WebSocket.MessageType.HEARTBEAT_REQUEST: {
					const heartbeatResponse =
						Communication.WebSocket.createMessage<Communication.WebSocket.HeartbeatResponseMessage>(
							{
								type: Communication.WebSocket.MessageType.HEARTBEAT_RESPONSE,
							},
						);
					ws.send(JSON.stringify(heartbeatResponse));
					break;
				}
				case Communication.WebSocket.MessageType.CLIENT_CONFIG_REQUEST: {
					if (!this.config) {
				...
				case Communication.WebSocket.MessageType.KEYFRAME_ENTITIES_REQUEST: {
					const request =
				...
				case Communication.WebSocket.MessageType.QUERY_REQUEST:
					await this.handleQuery(ws as ServerWebSocket<WebSocketData>, data);
					break;
				default: {
					const errorMsg =
						Communication.WebSocket.createMessage<Communication.WebSocket.ErrorResponseMessage>(
			...
			sessionsToCheck.map(async ([sessionId, session]) => {
				...
				if (
					...
					if (!validation?.is_valid) {
						log({
							prefix: this.LOG_PREFIX,
							message: "Session expired / invalid, closing WebSocket",
							debug: VircadiaConfig_Server.debug,
							type: "debug",
							data: {
								sessionId,
				...
				try {
					...
					const [result] = await this.sql.begin(async (sql) => {
						...
						const query = syncGroup
							? sql<[{ entity_scripts: Entity.Script.I_Script[] }]>`
...
export class WorldApiManager {
	...
	async initialize() {
		try {
			...
			const [configResult] = await this.sql<[{ value: Config.I_ConfigValue }]>`
			...
			this.restManager = new WorldRestManager(this.sql, configResult.value);

			this.wsManager = new WorldWebSocketManager(this.sql, configResult.value);
