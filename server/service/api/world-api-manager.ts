	...
	async initialize() {
		try {
			...
			superUserSql.subscribe("notification", async (notification) => {
				if (notification?.channel === "tick_captured" && notification.payload) {
					try {
				...
				websocket: {
					message: async (
						...
						try {
							...
							// Parse message
							data = JSON.parse(message) as Communication.WebSocket.Message;
							...
							switch (data.type) {
								case Communication.WebSocket.MessageType.QUERY_REQUEST: {
									...
									try {
										const results = await proxyUserSql?.begin(async (tx) => {
			...
			this.heartbeatInterval = setInterval(async () => {
				...
				await Promise.all(
					sessionsToCheck.map(async ([sessionId, session]) => {
						if (!superUserSql) {
							throw new Error("No super user database connection available");
						}
