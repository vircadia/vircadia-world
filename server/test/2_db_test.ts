...
async function initTestAccounts(): Promise<void> {
	await superUserSql.begin(async (tx) => {
		...
		expect(anonAuthProviderConfig.provider__session_duration_ms).toBeDefined();
		...
		describe("Session Management", () => {
			test("should verify we are using vircadia_agent_proxy", async () => {
				await proxyUserSql.begin(async (tx) => {
                    ...
                    `;
					expect(currentSessionUser.user).toBe("vircadia_agent_proxy");
					const [currentUser] = await tx`SELECT current_user`;
					expect(currentUser.current_user).toBe("vircadia_agent_proxy");
				});
			});
		...
		test("should read all config tables", async () => {
			await proxyUserSql.begin(async (tx) => {
				...
				const [networkConfig] = await tx`
                    ...
                    `;
				expect(networkConfig.network_config__max_latency_ms).toBeDefined();
				expect(networkConfig.network_config__warning_latency_ms).toBeDefined();
				expect(
				...
				const [authConfig] = await tx`
                    ...
                    `;
				expect(authConfig.auth_config__session_cleanup_interval).toBeDefined();
		...
		describe("Tick Operations", () => {
			...
			describe("Script Operations", () => {
				test("should create multiple test scripts and capture their tick states", async () => {
					await superUserSql.begin(async (tx) => {
						...
						const [script1] = await tx<[{ general__script_id: string }]>`
                    INSERT INTO entity.entity_scripts (
                        general__script_name,
                        group__sync
                    ) VALUES (
                ...
                `;
						const [script2] = await tx<[{ general__script_id: string }]>`
                    INSERT INTO entity.entity_scripts (
                        general__script_name,
                        group__sync
                    ) VALUES (
				...
				test("should detect script changes between ticks and only include changed fields", async () => {
					...
					await superUserSql.begin(async (tx) => {
						...
						const scriptChange = scriptChanges.find(
							(c) => c.general__script_id === script1.general__script_id,
						);
						...
						expect(scriptChange?.changes.compiled__browser__status).toBe(
							"PENDING",
						);

						// The URL field wasn't changed, so it shouldn't be included
						expect(scriptChange?.changes.source__repo__url).toBeUndefined();
					});
				});
			});
			...
			describe("Asset Operations", () => {
				test("should create multiple test assets and capture their tick states", async () => {
					await superUserSql.begin(async (tx) => {
						...
						const [asset1] = await tx<[{ general__asset_id: string }]>`
                    INSERT INTO entity.entity_assets (
                        general__asset_name,
                        group__sync,
                        asset__data,
                ...
                `;
						const [asset2] = await tx<[{ general__asset_id: string }]>`
                    INSERT INTO entity.entity_assets (
                        general__asset_name,
                        group__sync,
                        asset__data,
			...
			describe("Entity Operations", () => {
                ...
                `;
						...
						const states = await tx<Array<{ general__entity_id: string }>>`
