...
export namespace Server_CLI {
	...
	export async function runServerDockerCommand(data: {
		...
		const processEnv = {
			...
			VRCA_SERVER_SUPPRESS: serverConfiguration.VRCA_SERVER_SUPPRESS.toString(),
	...
	export async function restoreDatabase() {
		...
		try {
			...
			if (!existsSync(restoreFilePathHost)) {
				throw new Error(`Restore file not found: ${restoreFilePathHost}`);
			}
	...
	try {
		switch (command) {
			...
			case "client:set-uri": {
				...
				process.env.VRCA_CLIENT_WEB_BABYLON_JS_DEFAULT_WORLD_API_URI = newUri;
