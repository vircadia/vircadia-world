...
export namespace Client_CLI {
	...
	export async function runClientDockerCommand(data: {
		...
		let dockerArgs = ["docker", "compose", "-f", CLIENT_DOCKER_COMPOSE_FILE];
	...
	export async function isWebBabylonJsProdHealthy(data: {
		...
		const checkWebBabylonJs = async (): Promise<{
			...
			try {
				...
				return {
					isHealthy,
					error: isHealthy ? undefined : new Error("Service not responding"),
				};
			} catch (error) {
				return {
					isHealthy: false,
...
export namespace Server_CLI {
	...
	export async function runServerDockerCommand(data: {
		...
		const processEnv = {
			...
			VRCA_SERVER_DEBUG: VircadiaConfig_SERVER.VRCA_SERVER_DEBUG.toString(),
	...
	export async function wipeDatabase() {
		...
		try {
			...
			const systemResetDir =
				VircadiaConfig_CLI.VRCA_CLI_SERVICE_POSTGRES_SYSTEM_RESET_DIR;
			const userResetDir =
				VircadiaConfig_CLI.VRCA_CLI_SERVICE_POSTGRES_USER_RESET_DIR;
			const systemResetFiles = await readdir(systemResetDir, {
				recursive: true,
			});
			...
			const userResetFiles = await readdir(userResetDir, { recursive: true });
			...
			for (const file of resetSqlFiles) {
				try {
					...
					await sql.begin(async (sql) => {
						...
						await sql.unsafe("SET TRANSACTION ISOLATION LEVEL SERIALIZABLE");
		...
		try {
			...
			const processAssetFile = async ({
				...
				const matchingAssets = dbAssets.filter(
					(dbAsset) => dbAsset.general__asset_file_name === searchName,
				);
				...
				const fileExt = path.extname(fileName).toUpperCase().substring(1);
			...
			if (syncGroup) {
				...
				assetsToProcess = allAssetFileNames.filter((asset) =>
					syncGroupAssets.some((dbAsset) =>
						dbAsset.general__asset_file_name.includes(asset.searchName),
					),
				);
			}
			...
			if (options.parallelProcessing) {
				// Process in batches
				for (let i = 0; i < assetsToProcess.length; i += options.batchSize) {
					const batch = assetsToProcess.slice(i, i + options.batchSize);
			...
			const processAsset = async (asset: {
				...
				try {
					...
					if (asset.asset__data__bytea) {
						await writeFile(filePath, Buffer.from(asset.asset__data__bytea));
	...
	try {
		switch (command) {
			...
			case "server:postgres:system-token:invalidate-all": {
				...
				const invalidatedCount = await Server_CLI.invalidateDbSystemTokens();
				log({
			...
			case "server:postgres:seed:assets": {
				...
				await Server_CLI.seedAssets({
					options: {
						parallelProcessing:
							additionalArgs.length > 0 ? Boolean(additionalArgs[0]) : true,
						batchSize:
			...
			case "server:pgweb:access-command": {
				const pgwebAccessURL = await Server_CLI.generatePgwebAccessURL();
				log({
